...
class AdminFilters extends CControl{
	...
	{
		global $app;
		if(CForm::is_submit($this->formname, 'clear_'.$this->object_id))
		{
			$this->cache = array();
			$this->set_defaults();
		}
		elseif (CForm::is_submit($this->formname, 'filter_'.$this->object_id))
		{
			...
			if(preg_match('/^adminfilter_[a-zA-Z0-9_-]+#[a-zA-Z0-9_-]+$/', $field))
			{
				...
				switch ($this->filter_arr[$field]['condition'])
				{
					case CONDITION_EQUAL:
						if(strlen($value) > 0) $_where[$this->object_id] .= ' AND '.str_replace('#', '.', $field).'="'.mysql_escape_string($value).'"';
						break;
					case CONDITION_LIKE:
						if(strlen($value) > 0) $_where[$this->object_id] .= ' AND '.str_replace('#', '.', $field).' LIKE "'.mysql_escape_string($value).'%"';
						break;
				}
				if($this->filter_arr[str_replace('_from', '', $field)]['type'] == FILTER_DATE)
				{
					if(strlen($value) > 0) $_where[$this->object_id] .= ' AND '.str_replace('#', '.', str_replace('_from', '', $field)).' >= "'.mysql_escape_string($value).'"';
				}
				if($this->filter_arr[str_replace('_to', '', $field)]['type'] == FILTER_DATE)
				{
					if(strlen($value) > 0) $_where[$this->object_id] .= ' AND '.str_replace('#', '.', str_replace('_to', '', $field)).' <= "'.mysql_escape_string($value).'"';
				}
			}
			elseif(preg_match('/^adminfilter_[a-zA-Z0-9_-]+$/', $field))
			{
	...
	function set_defaults()
	{
		foreach ($this->filter_arr as $field => $data)
		{
			if(isset($data['default']) && strlen($data['default']) && !isset($this->cache[$field]))
			{
				$this->cache[$field] = $this->tv['adminfilter_'.$field] = $data['default'];
				switch ($this->filter_arr[$field]['condition'])
				{
					case CONDITION_EQUAL:
						$_where[$this->object_id] .= ' AND '.str_replace('#', '.', $field).'="'.mysql_escape_string($data['default']).'"';
						break;
					case CONDITION_LIKE:
						$_where[$this->object_id] .= ' AND '.str_replace('#', '.', $field).' LIKE "'.mysql_escape_string($data['default']).'%"';
						break;
				}
				if($this->filter_arr[str_replace('_from', '', $field)]['type'] == FILTER_DATE)
				{
					$_where[$this->object_id] .= ' AND '.str_replace('#', '.', str_replace('_from', '', $field)).' >= "'.mysql_escape_string($data['default']).'"';
				}
				if($this->filter_arr[str_replace('_to', '', $field)]['type'] == FILTER_DATE)
				{
					$_where[$this->object_id] .= ' AND '.str_replace('#', '.', str_replace('_to', '', $field)).' <= "'.mysql_escape_string($data['default']).'"';
				}
			}
		}
	}
