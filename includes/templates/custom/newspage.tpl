<? if ($news_found): ?>
	<section id="main">
        ...
        <ul class="bread_crumbs" itemprop="breadcrumb">
            ...
            <? if (!$news_uri): ?><li>Статьи</li><? else: ?><li><a href="<? echo $HTTP; ?>news.html">Статьи</a></li><li><? echo $news_arr[1]['title']; ?></li><? endif; ?>
        </ul>
        </div>
        <div class="row">
            <div class="twelve columns">
                <? if (!$news_uri): ?><h1 class="heading">Статьи</h1><? endif; ?>
				...
				<? foreach($news_arr as $item): ?>
					...
					<? $pos = strpos($item['description'], '<p><!-- more --></p>'); if ($pos != false) echo substr($item['description'], 0, $pos); else  echo $item['description_main']; ?>


                    <div class="article_more"><a href="<? echo $HTTP.'news/'.$item['uri'].'.html'; ?>">Подробнее</a></div>
					</div>
					<? endforeach; ?>
                 </div>
				</div>
				<? else: ?>
					...
					<div class="top_article">
						<span class="price"><? echo $news_arr[1]['title']; ?></span>
					</div>
					<div class="article_date">
						<span class="public_date"><strong><? echo $news_arr[1]['public_date']; ?></strong></span>
					</div>
					<?
					if (!empty($news_arr[1]['image_filename']))
						echo '<img class="news_img" src="'.$HTTP.'pub/articles/'.$news_arr[1]['id'].'/'.$news_arr[1]['image_filename'].'" alt="'.$news_arr['title'].'" />';
					?>
					<? echo $news_arr[1]['description']; ?>
                </div>
				</div>
                        ...
                        <div class="twelve columns">
                            ...
                            <div class="white_block">
                                ...
                                <div class="review_form" id="review_form">
                                    ...
                                    <? CForm::begin('add_comment', 'post', 'http://'.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI], 'f-form'); ?>
