...
(function() {
	function Selection(selection) {
		...
		this.addRange = function(rng) {
			...
			// If single element selection then try making a controls selection out of it
...
(function(tinymce) {
	...
	tinymce.create('tinymce.dom.Selection', {
		...
		getRng : function(w3c) {
			...
			// We have W3C ranges and it's IE then fake controls selection since IE9 doesn't handle that correctly yet
		...
		getNode : function() {
			...
			if (rng.setStart) {
				...
				// Handle selection a image or other controls like element such as anchors
				if (!rng.collapsed) {
	...
	tinymce.dom.RangeUtils.compareRanges = function(rng1, rng2) {
		if (rng1 && rng2) {
			...
			if (rng1.item || rng1.duplicate) {
				// Both are controls ranges and the selected element matches
...
tinymce.create('tinymce.ui.Toolbar:tinymce.ui.Container', {
	renderHTML : function() {
		...
		for (i=0; i<cl.length; i++) {
			// Get current controls, prev controls, next controls and if the controls is a list box or not
			co = cl[i];
			pr = cl[i - 1];
			nx = cl[i + 1];
...
(function(tinymce) {
	...
	tinymce.create('tinymce.Editor', {
		...
		init : function() {
			...
			// Enables users to override the controls factory
		...
		focus : function(sf) {
			...
			if (!sf) {
				...
				// Restore selected controls element
				// This is needed when for example an image is selected within a
				// layer a call to focus will then remove the controls selection
			...
			if (s.custom_undo_redo) {
				...
				t.onKeyDown.add(function(ed, e) {
					...
					if (keyCode == 8) {
						...
						// Fix IE controls + backspace browser bug
		...
		setup : function() {
			...
			if (s.forced_root_block) {
				function addRootBlocks() {
					...
					if (rng.setStart) {
						...
						// Force controls range into text range
						if (rng.item) {
			...
			if (s.force_p_newlines) {
				if (!isIE) {
					...
					// custom logic that enables us to controls the output
					tinymce.addUnload(function() {
