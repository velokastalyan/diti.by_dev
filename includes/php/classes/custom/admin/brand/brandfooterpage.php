...
class CBrandFooterPage extends CMasterPage
{
	/**
	 * The table name.
	 *
	...
	function CBrandFooterPage(&$app, $template)
	{
		$this->IsSecure = true;
	...
	function bind_data()
	{
		$query = "SELECT
        		...
        		title,
        		link,
        		IF(LENGTH(image_filename) > 0, CONCAT('<img src=\"".$this->tv['HTTP']."pub/brands/footer/', id, '/75x75/',image_filename,'\">'), 'Изображение отсутсвует') as image
            FROM 
            	%prefix%brand_footer
            WHERE ".$this->_where;
		...
		$header_num = $nav->add_header('link');
		$nav->headers[$header_num]->set_title('Ссылка');
