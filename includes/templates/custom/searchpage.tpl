	<section id="main">
        ...
        <div class="row">
            <div class="twelve columns">
                <h1 class="heading">Результат поиска</h1>

					<? if ($search_found): ?>
                    <ul class="list_products">
						<? foreach($search_arr as $item1): ?>
                        <li>
                            <a href="<? echo $HTTP; ?><? echo $item1['parent_path_uri']; ?>/<? echo $item1['c_uri']; ?>/<? echo $item1['uri']; ?>.html">
								<? if ($item1['is_sale'] > 0): ?>
					 ...
					 <? else: ?>
						<div class="white_block indent">
							<i>По данному запросу ничего не найдено</i>
						</div>
					<? endif; ?>

				<? CControl::process('Pager', 'Search'); ?>
            </div>
        </div>
