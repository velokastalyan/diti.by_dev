...
class CSnoubordPage extends CFrontPage  {
    ...
    function on_page_init(){
        ...
        $this->tv['category'] = InGet('category', -1);
    ...
    function bind_category()
    {
        ...
        $rs = $this->Categories->get_by_uri_with_children('snoubord-deki','snoubord');
        ...
        if($rs !== false && !$rs->eof())
        {
            ...
            $this->PAGE_TITLE = $rs->get_field('meta_title');
            //$this->PAGE_KEYWORDS = $rs->get_field('meta_keywords');
            $this->PAGE_DESCRIPTION = $rs->get_field('meta_description');
            ...
            if(intval($rs->get_field('child_id')))
            {
                ...
                while (!$rs->eof())
                {
                    ...
                    if($rs->get_field('child_uri') == $this->c3_uri)
                    {
                        ...
                        $this->PAGE_TITLE = $rs->get_field('child_meta_title');
                        //$this->PAGE_KEYWORDS = $rs->get_field('child_meta_keywords');
                        $this->PAGE_DESCRIPTION = $rs->get_field('child_meta_description');
                    }
                    $rs->next();
                }
            }
    ...
    switch($this->tv['year'])
{
    ...
    default: $this->tv['year'] = -1; break;
        ...
        $product_rs = $this->Products->get_snoubord_products(array('c1_uri' => 'snoubord', 'c2_uri' => 'snoubord-deki'), $this->sort_by, $page, $this->on_page, $this->tv['brand'], $price[0], $price[1], $this->tv['sex'], $this->tv['category'],$this->tv['year']);
        //print_arr($product_rs);
        if ($product_rs != false) {
        ...
        $all_product_rs = $this->Products->get_snoubord_products(array('c1_uri' => 'snoubord', 'c2_uri' => 'snoubord-deki'), $this->sort_by, 1, 'all', $this->tv['brand'], $price[0], $price[1], $this->tv['sex'], $this->tv['category'],$this->tv['year']);

        if ($all_product_rs != false) {

            while(!$all_product_rs->eof())
            {
                row_to_vars($all_product_rs, $this->tv['all_product_arr'][count($this->tv['all_product_arr'])+1]);
                $all_product_rs->next();
            }
        }
    ...
    function bind_brand_for_products()
    {
        ...
        if ($price[0] == '1501000') $price[1] = '99999999999';
            ...
            $brand_rs = $this->Brands->get_snoubord_brands('snoubord/snoubord-deki', $this->tv['sex'], $price[0], $price[1]);
        ...
        if ($brand_rs !=false)
        {
            ...
            while (!$brand_rs->eof())
            {
                row_to_vars($brand_rs, $this->tv['brand_arr'][count($this->tv['brand_arr'])]);
                $brand_rs->next();
            }
        }
    }
    ...
    9 => array(
        ...
        for ($i = 0; $i < count($sex_rs); $i++)
        {
            for ($j = 1; $j < count($this->tv['all_product_arr']) +1; $j++)
            {
                if ($this->tv['all_product_arr'][$j]['sex'] == $sex_rs[$i]['id'])
                {
                    ...
                    if (!$d)
                        row_to_vars($sex_rs[$i], $this->tv['sex_arr'][count($this->tv['sex_arr'])]);
                }
            }
        }
        ...
        for ($i = 0; $i < count($price_rs); $i++)
        {
            ...
            if (count($this->tv['all_product_arr']) > 1)
                for ($j = 1; $j < count($this->tv['all_product_arr']) +1; $j++)
                {
                    if ($this->tv['all_product_arr'][$j]['discount'] == 0)
                    {
                        if (($this->tv['all_product_arr'][$j]['price'] >= $prices[0]) && ($prices[1] >= $this->tv['all_product_arr'][$j]['price']))
                        {
                            ...
                            if (!$d)
                                row_to_vars($price_rs[$i], $this->tv['price_arr'][count($this->tv['price_arr'])]);
                        }
                    }
                    else
                    {
                ...
                if ($this->tv['all_product_arr'][1]['discount'] == 0)
                {
                    if (($this->tv['all_product_arr'][1]['price'] >= $prices[0]) && ($prices[1] >= $this->tv['all_product_arr'][1]['price']))
                    {
                        row_to_vars($price_rs[$i], $this->tv['price_arr'][count($this->tv['price_arr'])]);
                    }
                }
                ...
                {
                    if (($this->tv['all_product_arr'][1]['discount'] >= $prices[0]) && ($prices[1] >= $this->tv['all_product_arr'][1]['discount']))
                    {
                        row_to_vars($price_rs[$i], $this->tv['price_arr'][count($this->tv['price_arr'])]);
                    }
                }
