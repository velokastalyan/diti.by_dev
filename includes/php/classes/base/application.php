...
require_once('components/memcached.php');
...
class CApplication {
	...
	/**
	 * @var array - array for application's modules
	 */
	protected $Modules;
	/**
	 * @var CDataBase - class for DataBase
	 */
	public $DataBase;

	/**
	 * @var CUser - users module
	 */
	public $User;

	/**
	 * @var CRouter - class Router
	 */
	public $Router;

	/**
	 * @var CLocalizer - class for localization
	 */
	public $Localizer;

	/**
	 * @var CurrentPage - class for work with current page
	 */
	public $CurrentPage;

	/**
	 * @var $template_vars array - template variables DEPRECATED use by alias TV
	 */
	public $template_vars;

	/**
	 * @var $tv array - template variables
	 */
	public $tv;
	public $events;
	public $locale;
	...
	/**
	 * @param $name
	 * @return bool|value from $_SERVER[$name]
	 */
	...
	/**
	 * add classes of custom modules to return params, returns reference to the module
	 * @param string $mod_name BF|Thumbnail|DataBase|User|Localizer|Router|Registry|Navi
	 * @param bool $ajax_check
	 * @return CBlowFish|CThumbnail|CUser|CLocalizer|CRouter|CRegistry|CNavi|CImages
	 */
	function &get_module($mod_name, $ajax_check = false)
	{
		if (isset($this->Modules[$mod_name])){
			if (is_object($this->Modules[$mod_name]) && (!$ajax_check || (bool)$this->Modules[$mod_name]->ajax_visible === true))
				return $this->Modules[$mod_name];
			elseif (is_array($this->Modules[$mod_name]) && (!$ajax_check || (bool)$this->Modules[$mod_name]['AjaxVisible'] === true)) {
