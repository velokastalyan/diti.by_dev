...
class CDataBase extends CBaseDB
{
        ...
        // FIX: Отключение строгого режима (исправление ошибки сохранения)
...
?>docker exec -i ditiby-web-1 php -r '
$src = "/var/www/html/includes/php/classes/base/database.mysql.php";
$dest = "/var/www/html/database_FIXED_2.php"; // <--- Новая версия файла

if (!file_exists($src)) { die("Исходный файл не найден!"); }

$code = file_get_contents($src);

// Ищем mysql_select_db и оборачиваем в фигурные скобки { ... } вместе с нашим фиксом
// Это безопасно для любых if/else конструкций
$code = preg_replace(
    "/(mysql_select_db\s*\([^;]+;)/i", 
    "{ $1 mysql_query(\"SET SESSION sql_mode = \x27\x27\"); }", 
    $code
);

file_put_contents($dest, $code);
echo "✅ Готово! Создан безопасный файл database_FIXED_2.php\n";
'