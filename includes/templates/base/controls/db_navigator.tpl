<div class="nav">
	<? if($navigator_is_empty): ?>
		<div class="alert">
			<h5><? echo $title; ?></h5> <span class="stt">(<? CDeprecatedTemplate::loc_string('nav_empty_message'); ?>)</span>
			<div class="clear">&nbsp;</div>
		</div>
	<? else: ?>
		...
		<div class="clear">&nbsp;</div>
		<div class="heading">
			<span class="ttl"><? echo $title; ?></span> <span class="stt">(<? echo $total_items; ?>  <? CDeprecatedTemplate::loc_string('total'); ?>, <? CDeprecatedTemplate::loc_string('displaying'); ?> <? echo $row_numb_before; ?><? if($size > 1): ?> <? CDeprecatedTemplate::loc_string('to'); ?> <? echo $row_numb_end; ?><? endif; ?>)</span>
		...
		<div class="nav_body">
			...
			<tbody><tr class="nav_t_ttl">
				<? foreach ($headers as $field => $data): ?>
					...
					<? else: ?>
						<td width="<? echo $data['width']; ?>" <? if(!$data['wrap']): ?>nowrap="nowrap"<? endif; ?> align="<? echo $data['align']; ?>" valign="<? echo $data['valign']; ?>" class="t_cl1_ttl <? if($data['sort_act']): ?>sort_<? echo $data['sort_act']; ?><?endif;?>" <? if($data['is_sort']): ?>onclick="gotoURL('<? echo $data['sort_link']; ?>');"<? endif; ?>>
							<? if($data['is_sort']): ?>
								<a href="<? echo $data['sort_link']; ?>" title="<? CDeprecatedTemplate::loc_string('click_for_sort'); ?>"><? echo $data['title']; ?></a>
							<? else: ?>
					...
					<? foreach ($headers as $field => $data): ?>
						...
						<? else: ?>
							<? if($data['clickable'] && $is_checkable): ?>
								<td valign="<? echo $data['valign']; ?>" align="<? echo $data['align']; ?>" class="hand nav_row" <? if (substr_count($_SERVER['REQUEST_URI'], 'recommend_edit') == 0): ?>onclick="gotoURL('<? echo $data['clckaction']; ?><? echo $row['id']; ?>');"<? endif; ?> <? if(!$data['wrap']): ?>nowrap="nowrap"<? endif; ?>>
							<? else: ?>
								<td valign="<? echo $data['valign']; ?>" align="<? echo $data['align']; ?>" class="hand nav_row" nowrap="<? echo $data['wrap']; ?>">
							<? endif;?>
							<? if($data['mail']): ?>
								<a href="mailto:<? echo $row[$field]; ?>" title="<? CDeprecatedTemplate::loc_string('click_for_send_message'); ?>"><? echo $row[$field]; ?></a>
							<? elseif($data['swapPosition'] && $dbposition_show): ?>
								<a href="#" class="popup-open popup-swappos" dbnavigator="<? echo $object_id; ?>" title="<? CDeprecatedTemplate::loc_string('open_swapposition_popup'); ?>">&nbsp;</a>
								<? echo $row[$field]; ?> 
								<div class="clear">&nbsp;</div>
							<? else: ?>
								<? echo $row[$field]; ?>
...
<? if($dbposition_show): ?>
	...
	<div id="<? echo $object_id; ?>_dbposition" class="popupblock">
		...
		<div class="bg">&nbsp;</div>
			<div class="cont">
				<h1><span id="popup-title"><? CDeprecatedTemplate::loc_string('Position'); ?></span> <a href="#" class="close popup-swappos"><? CDeprecatedTemplate::loc_string('close'); ?></a></h1>
						...
						<? if($dbposition_filters): ?>
							<? foreach ($dbposition_filters as $field => $text): ?>
								<div class="field">
									...
									<? CDeprecatedTemplate::input('select', "{$field}_dbposition", "{$field}", 'sel sortable_select'); ?>
								</div>
							<? endforeach; ?>
						<? endif; ?>
						<div class="field">
							<label for="pos_start"><? CDeprecatedTemplate::loc_string('start_pos'); ?></label>
							<input type="text" id="pos_start" name="pos_start" class="inp inp25 pos_start" />
						</div>
						<div class="field">
							<label for="pos_end"><? CDeprecatedTemplate::loc_string('end_pos'); ?></label>
							<input type="text" id="pos_end" name="pos_end" class="inp inp25 pos_end" />
						</div>
						<div class="field right_block">
							<div class="finpwrapper"><input type="button" name="pos_clear" value="<? CDeprecatedTemplate::loc_string('clear'); ?>" class="fbutt pos_clear" /></div>
							<div class="finpwrapper"><input type="button" name="pos_filter" value="<? CDeprecatedTemplate::loc_string('sort'); ?>" class="fbutt pos_filter" /></div>
						</div>
					</div>
