...
class CInputs
{
	...
	function generate_uri_ereg($string) {
		...
		$str = preg_replace('/[^a-z0-9]{1,}/', '-', strtolower(translit($string)));
		if ($str[strlen($str) - 1] == "-") {
	...
	function delete_rp($id)
	{
		...
		$children_rs = $this->DataBase->select_sql('registry_path', array('parent_id' => $id));
		...
		if(!$children_rs->eof())
		{
			while(!$children_rs->eof())
			{
				if(!$this->delete_rp($children_rs->get_field('id')))
					return false;
				$children_rs->next();
			}
		}
