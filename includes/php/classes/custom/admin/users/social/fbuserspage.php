...
class CFbUsersPage extends CMasterPage
{
    /**
     * The table name.
     *
	...
	function CFbUsersPage(&$app, $template)
	{
		$this->IsSecure = true;
	    ...
	    $this->_filters = array(
	    	'm#name' => array(
	    		'title' => $this->Application->Localizer->get_string('full_name'),	
	    		'type' => FILTER_TEXT,
	    		'data' => null,
	...
	function bind_data()
	{
        $query = "SELECT m.id as id,
            ...
            FROM %prefix%fb_user AS m LEFT JOIN %prefix%user_role ml ON (m.user_role_id = ml.id)
            WHERE ".$this->_where."";
		
        require_once(BASE_CLASSES_PATH . 'controls/dbnavigator.php');
        $nav = new DBNavigator('fb_user', $query, array('status', 'name'), 'id');
